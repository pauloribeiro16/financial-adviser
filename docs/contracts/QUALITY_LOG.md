...
| S12-P1 | 2026-07-03 | 9/9            | 2/2             | 3/3        | PASS    | **Port Langfuse prompts registry to master** (base `origin/master@7a88010`). New `app/prompts/{__init__,registry,sync}.py` (138 + 144 LOC). Registry delegates disk fallback to `engine.persona_system_prompt` (so master's 6-pillar structure flows through). 20 new tests in `tests/test_prompts_registry.py`. 18 prompts pushed to Langfuse. 0 regressions on master's 23 existing tests. |
| S12-P2 | 2026-07-03 | 12/12          | 2/2             | 3/3        | PASS    | **LangGraph StateGraph adapted to master engine**. New `app/debate/graph.py` (283 LOC) with 7 nodes + `Send()` fan-out + `MemorySaver`. LLM calls use master's `engine._invoke_with_fallback` (3-level safety net, slim schemas). `sector` param flows through to `engine.build_*_messages`. `orchestrator.py` routes: debate mode → `graph.invoke()`, legacy `--format {md,json,per-agent}` → `engine.run_debate()`. 18 new tests in `tests/test_debate_graph.py`. 43/43 master regression tests pass. |
| S12-P3 | 2026-07-03 | 9/9            | 2/2             | 3/3        | PASS    | **Coherent tracing + auto session_id (port)**. `_auto_session_id` in `main.py` (`debate-{domain}-{target}-{YYYYMMDD_HHMMSS}`); defensive auto-gen in `orchestrator.py`. `@observe(name=, as_type=)` on all 7 graph nodes. `_NoopHandler` now subclasses `BaseCallbackHandler`. 15 new tests (7 `test_session_id.py` + 8 `test_graph_decorators.py`). Live Langfuse: 1 trace per debate with session_id + 102 observations correctly nested under one root. |
| S13    | 2026-07-03 | 6/6            | 2/2             | 3/3        | PASS    | Docs + env + CLI cleanup. README + AGENTS: `python -m` → `python3 -m`. `.env`: `MINIMAX_MODEL=claude-sonnet-4-5` → `MiniMax-M3`. Removed `--format debate` from `app/main.py` argparse choices (default None; debate is implicit for `--company`/multi-round). `app/cli_menu.py`: format question skipped when in debate path. `orchestrator.py`: dead `format` param removed. 2 commits on `sprint/s13-docs-cli-cleanup`. 0 regressions; 10 pre-existing failures unchanged. |
| S14-P1 | 2026-07-03 | 11/11          | 2/2             | 3/3        | PASS    | Surveillance structure. New `app/sectors/` (4 YAMLs: Energy, Technology, Healthcare, Financial-Services — 6-8 indicators each, loaded as Pydantic IndicatorSpec). New `app/watch/{reference_reader,price_target,cli}.py` + package marker. `load_latest_debate()` picks max-by-TS in `out/<sector>/<ticker>/`. `compute_buy_price` heuristic: fair × (company_fcf_yield/sector_target_fcf_yield) × margin (0.79-0.91 by moat). 47 new tests. 262 passes; 10 pre-existing failures unchanged. |
| S14-P2 | 2026-07-03 | 11/11          | 2/2             | 3/3        | PASS    | Surveillance aggregation. New `app/watch/{aggregator,markdown_renderer,sector_runner,sector_file,langfuse_tracing}.py`. `WatchSummary` Pydantic (5 fields, max-length caps) + `aggregate_one` via `engine._invoke_with_fallback` (3-level safety net) + 1 retry with repair prompt. `render_company_current_md` (5 bullets + indicators table + buy target + trail). `surveillance_trace(sector)` with session_id `surveillance-<sector>-<ts>`, tags `phase:surveillance`. 17 new tests. End-to-end smoke: real XOM debate → current.md (5 bullets + 7 indicators + buy target $124.75 + trail) + _index.md. 279 passes. |
| S14-P3 | 2026-07-03 | 9/9            | 2/2             | 3/3        | PASS    | CLI integration. `app/main.py`: added `watch` subparser (`--sector`, `--ticker`, `--all`, `--provider`, `--output`) with backward-compat debate flow preserved. `app/watch/cli.py`: full `cmd_watch` impl. `app/cli_menu.py`: TOP-LEVEL mode picker (Analyze vs Watch). 10 new tests in `tests/test_cli_watch.py`. Live: `python3 -m app.main watch --sector Energy --provider mock` produces `out/surveillance/energy/XOM/current.md` + `_index.md` + `history.json`. Interactive menu reaches mode → sector → cmd_watch. 289 passes; 10 pre-existing failures unchanged. |
| S15    | 2026-07-03 | 11/11          | 2/2             | 3/3        | PASS    | Aggregator prompt hardening (5 improvements). `app/watch/aggregator.py`: `CyclePhase` StrEnum (Hyper-Growth, Operating Leverage, Capital Return, Decline); `WatchSummary.cycle_phase` typed as enum; `field_validator` hard-rejects moat without leading 🟢/🟡/🔴; fundamentals kwarg injects structured snapshot into user message; 14-item DO NOT block (hedge language, invented numbers, platitudes, embedded lists, metacomm, recommendations, disclaimers, ticker repetition, synonym stacking, filler connectors, padding); 1-shot exemplar based on real XOM debate. `app/watch/sector_runner.py`: passes extracted indicators into aggregate_one. `app/providers.py`: deferred-import for WatchSummary in MockModel (avoids cycle). 11 new tests. 300 passes; 10 pre-existing failures unchanged. |
| S16-P1 | 2026-07-03 | 16/16          | 2/2             | 3/3        | PASS    | Filings infrastructure (scope-locked). `app/filings/{fetcher,section_parser,cache}.py`: download 10-K HTML with retry, regex-based section extraction (Item 1/1A/7/7A), permanent JSON cache in `data/cache/filings/`. `app/models.py`: +9 LOC for `FilingSummary` (4 fields, max_length=500). 16 new tests (8 parser + 8 cache). FICH-LOCKED respected: zero files outside the allow list. 316 passes; 10 pre-existing unchanged. |
| S16-P2 | 2026-07-03 | 8/8            | 2/2             | 3/3        | PASS    | Filings summarization. `app/filings/{summarizer,prompts}.py`: 3 LLM calls via `engine._invoke_with_fallback` (re-uses master's 3-level safety net); map-reduce if section >8K words (4K chunks + 200-word overlap + consolidation prompt); `get_or_build_summary` cache-first → download → parse → summarize → cache. 8 new tests. 324 passes; 10 pre-existing unchanged; 0 regressions. |
| S16-P3 | 2026-07-03 | 5/5            | 2/2             | 3/3        | PASS    | CLI flag + context integration. `app/main.py`: `--with-filings` flag (default False). `app/pipeline/context.py`: `build_company_context(ticker, *, with_filings=False)` adds filing_summary if True; `_render_company` emits `## 10-K Narrative Summary` section when present. `app/cli_menu.py`: interactive menu toggle. 5 new tests in `tests/test_filings_integration.py`. FICH-LOCKED respected within strict LOC budgets (+20/+5/+10). 329 passes; 10 pre-existing unchanged; 0 regressions. |
| S16-P4 | 2026-07-03 | 5/5            | 2/2             | 3/3        | PASS    | Plumb + passive cache. `app/main.py` (+12): both `run_debate_only()` call sites now build `ctx` via `build_company_context(tgt, with_filings=True)` when flag set, pass as `ctx=`; orchestrator's existing `ctx` param skips `_build_context()`. `app/pipeline/context.py` (+6): `build_company_context` reads filings cache passively when `with_filings=False` — if cache hit, include; if miss, graceful fallthrough. Forced build only when flag=True. Behavior matrix: `--with-filings + cache hit` reuse cache; `--with-filings + miss` download+build; no-flag + hit include from cache (NEW); no-flag + miss debate unchanged. 329 passes; 0 regressions. |
